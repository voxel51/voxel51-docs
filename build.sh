...
# readonly NODE_VERSION=17.9.0  # commenting because "SC2034 (warning): NODE_VERSION appears unused. Verify use (or export if used externally)."
...
main() {
  ...
  if [ $SKIP_PYTHON_API -eq 0 ]; then
    ...
    if [ ! -d "$API_DOC_DIR" ] || ! ls "$API_DOC_DIR"/*.html >/dev/null 2>&1; then
      ...
      # commenting because "SC2154 (warning): pydoctor_output is referenced but not assigned"
